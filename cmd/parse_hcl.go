		...
		Functions: map[string]function.Function{
			"find_in_parent_folders": function.New(&function.Spec{
				...
				Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
					// Default to looking for terragrunt.hcl
					fileName := "terragrunt.hcl"
					if len(args) > 0 && args[0].Type() == cty.String {
						fileName = args[0].AsString()
					}
					
					// Start from the directory containing the current config
					...
					for {
						...
						parentFile := filepath.Join(parentDir, fileName)
						if _, err := os.Stat(parentFile); err == nil {
							// Found the parent file
							return cty.StringVal(parentFile), nil
						}
			...
			"get_path_from_repo_root": function.New(&function.Spec{
				Params: []function.Parameter{},
				Type:   function.StaticReturnType(cty.String),
				Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
					...
					rel, err := filepath.Rel(wd, filepath.Dir(configPath))
					if err != nil {
						return cty.StringVal("."), nil
					}
